...
entityMaker.prototype.findMapping = function(tableName, field){
	//检查是否在选项中有自定义映射
	var xPath = strformat('/.{0}.{1}', tableName, field.name);
...
entityMaker.prototype.makeOne = function(schema, sql){
	...
	schema.fields.forEach(function(item){
		...
		//增加静态字段
		field.staticField = self.upperFirstLetter(field.propertyName);
		//主键不会显示在字段列表中
		if(field.unique){
...
//从配置文件中读取
exports.makeWithConfig = function(cfgFile, callback){
	...
	//如果sqlite是字符，则直接读取sqlite数据库
	if(typeof(cfg.sqlite) == 'string'){
		exports.makeWithSqlite(cfg.sqlite, saveToFolder, cfg.options, function(){
			if(callback) callback();
		});
	}else if(cfg.sqlite instanceof Array){
		//处理sql语句
		exports.makeWithSql(cfg.sqlite, saveToFolder, cfg.options);
		if(callback) callback();
	}else{
		console.log("配置文件错误！");
	}
