...
//执行某个任务
entityMaker.prototype.run = function(sql){
...
//从配置文件中读取
exports.makeForConfig = function(cfgFile){
	...
	var maker = new entityMaker(saveToFolder, cfg.options);
	//使sqlite，通过读取sqlite获取sqlite语句
	if(cfg.sqlite){

	}else{
		//读取sql语句
		cfg.sqlist.forEach(function(sql){
			maker.run(sql);
		});
	}
}